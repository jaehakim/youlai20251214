...
## youlai 웹 시스템 적용 가능 Skills

youlai 관리자 시스템에서 구현할 수 있는 기능들을 카테고리별로 정리합니다.

### 1. 데이터 관리 Skills

| Skill | 설명 | 적용 화면 |
|-------|------|----------|
| 일괄 등록 (Excel Import) | Excel 파일로 데이터 대량 업로드 | 사용자 관리, 부서 관리, 사전 관리 |
| 일괄 내보내기 (Excel Export) | 조회 결과를 Excel로 다운로드 | 모든 목록 화면 |
| 휴지통/복원 | 논리 삭제된 데이터 복원 기능 | 사용자 관리, 메뉴 관리 |
| 데이터 복사 | 기존 데이터 복제하여 새로 생성 | 역할 관리, 메뉴 관리 |
| 드래그 정렬 | 마우스 드래그로 순서 변경 | 메뉴 관리, 부서 관리 |
| 버전 이력 관리 | 데이터 변경 히스토리 추적 | 설정 관리, 사용자 관리 |

### 2. 검색/필터 Skills

| Skill | 설명 | 적용 화면 |
|-------|------|----------|
| 고급 검색 | 다중 조건 검색 폼 | 로그 조회, 사용자 관리 |
| 검색 조건 저장 | 자주 쓰는 검색 조건 북마크 | 모든 목록 화면 |
| 실시간 필터링 | 입력 시 즉시 결과 필터 | 사전 항목, 메뉴 트리 |
| 날짜 범위 필터 | 기간 선택 검색 | 로그 조회, 알림 내역 |
| 태그 기반 필터 | 태그로 빠른 필터링 | 공지사항, 사용자 관리 |

### 3. UI/UX Skills

| Skill | 설명 | 적용 화면 |
|-------|------|----------|
| 다크 모드 | 어두운 테마 전환 | 전체 시스템 |
| 컬럼 커스터마이징 | 표시 컬럼 선택/순서 변경 | 모든 그리드 화면 |
| 컬럼 너비 저장 | 조정한 컬럼 너비 기억 | 모든 그리드 화면 |
| 키보드 단축키 | 빠른 작업 실행 | 전체 시스템 |
| 반응형 레이아웃 | 모바일/태블릿 최적화 | 전체 시스템 |
| 데이터 프리뷰 | 행 호버 시 상세 미리보기 | 사용자 목록, 역할 목록 |

### 4. AI/자동화 Skills

| Skill | 설명 | 적용 화면 |
|-------|------|----------|
| AI 기반 추천 | 역할/권한 자동 추천 | 사용자 역할 할당 |
| 자동 완성 | 입력 필드 자동완성 | 검색 필드, 폼 입력 |
| 이상 탐지 | 비정상 로그인/접근 감지 | 로그 조회, 대시보드 |
| 스마트 알림 | 중요도 기반 알림 분류 | 알림 센터 |
| 자연어 검색 | 자연어로 데이터 검색 | 검색 바 |

### 5. 알림/커뮤니케이션 Skills

| Skill | 설명 | 적용 화면 |
|-------|------|----------|
| 실시간 알림 | WebSocket 기반 푸시 | 알림 센터, 헤더 |
| 이메일 알림 | 중요 이벤트 메일 발송 | 백그라운드 서비스 |
| SMS 알림 | 긴급 알림 문자 발송 | 보안 이벤트 |
| 공지사항 팝업 | 로그인 시 공지 표시 | 대시보드 |
| 읽음 확인 | 공지사항 읽음 상태 추적 | 공지사항 관리 |

### 6. 보안 Skills

| Skill | 설명 | 적용 화면 |
|-------|------|----------|
| 2단계 인증 (2FA) | OTP/SMS 추가 인증 | 로그인 |
| 비밀번호 정책 | 복잡도/만료 규칙 설정 | 사용자 관리 |
| 로그인 이력 조회 | 사용자별 접속 기록 | 사용자 상세 |
| IP 화이트리스트 | 허용 IP 대역 관리 | 보안 설정 |
| 세션 관리 | 동시 로그인 제어 | 보안 설정 |
| 민감 데이터 마스킹 | 개인정보 마스킹 표시 | 사용자 목록 |

### 7. 리포트/분석 Skills

| Skill | 설명 | 적용 화면 |
|-------|------|----------|
| 대시보드 위젯 | 통계 요약 카드/차트 | 대시보드 |
| 사용자 활동 분석 | 접속/사용 패턴 분석 | 분석 화면 |
| 시스템 모니터링 | 서버 상태/성능 모니터링 | 시스템 관리 |
| 리포트 생성기 | 맞춤형 리포트 생성 | 리포트 화면 |
| 차트 커스터마이징 | 차트 유형/데이터 선택 | 대시보드 |

### 8. 워크플로우 Skills

| Skill | 설명 | 적용 화면 |
|-------|------|----------|
| 승인 프로세스 | 등록/수정 승인 단계 | 사용자 등록, 권한 변경 |
| 작업 예약 | 배치 작업 스케줄링 | 시스템 관리 |
| 자동 정리 | 오래된 로그/임시 파일 삭제 | 백그라운드 작업 |
| 상태 전이 | 데이터 상태 변경 흐름 | 사용자 상태 관리 |

### 9. 시스템 관리 Skills

| Skill | 설명 | 적용 화면 |
|-------|------|----------|
| 캐시 관리 | Redis 캐시 조회/삭제 | 시스템 설정 |
| 설정 관리 | 동적 시스템 설정 변경 | 설정 화면 |
| API 모니터링 | API 호출 현황/응답시간 | 모니터링 화면 |
| 스케줄러 관리 | 배치 작업 관리 | XXL-Job 연동 |
| 백업/복원 | 데이터베이스 백업 관리 | 시스템 설정 |

### 10. 코드 생성 Skills

| Skill | 설명 | 적용 화면 |
|-------|------|----------|
| CRUD 코드 생성 | 테이블 기반 코드 자동 생성 | 코드 생성기 |
| API 문서 생성 | OpenAPI 스펙 자동 생성 | Knife4j |
| TypeScript 타입 생성 | API 응답 타입 자동 생성 | 개발 도구 |
| 데이터 모킹 | 테스트용 더미 데이터 생성 | 개발 환경 |
