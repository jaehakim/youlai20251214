...
export type IToolsButton = {
  name: string; // 버튼이름칭
  text?: string; // 버튼텍스트
  perm?: Array<string> | string; // 권한标识(可以예完整권한문자열예'sys:user:add'또는작업권한예'add')
...
export interface ISearchConfig {
  // 권한접두사(예sys:user，용도그룹成권한标识)，아님提供그러면아님进행권한검사
  permP참조ix?: string;
  // 태그冒号(기본값：false)
  colon?: boolean;
  // 양식항목(기본값：[])
  ...
  // form컴포넌트속성
  form?: IForm;
  // 自适应网格레이아웃(사용시양식아님해야추가 style: { width: "200px" })
...
export interface IContentConfig<T = any> {
  ...
  pagination?:
    ...
    | Partial<
        ...
        >
      >;
  // 목록의네트워크요청함수(필요돌아가기promise)
  index액션: (queryParams: T) => Promise<any>;
  ...
  // 수정속성의네트워크요청함수(필요돌아가기promise)
  modify액션?: (data: {
    [키: string]: any;
    field: string;
    value: boolean | string | number;
  }) => Promise<any>;
  // 삭제의네트워크요청함수(필요돌아가기promise)
  delete액션?: (ids: string) => Promise<any>;
  // 백엔드내보내기의네트워크요청함수(필요돌아가기promise)
  export액션?: (queryParams: T) => Promise<any>;
  // 프론트엔드全양내보내기의네트워크요청함수(필요돌아가기promise)
  exports액션?: (queryParams: T) => Promise<IObject[]>;
  // 가져오기템플릿
  importTemplate?: string | (() => Promise<any>);
  // 백엔드가져오기의네트워크요청함수(필요돌아가기promise)
  import액션?: (file: File) => Promise<any>;
  // 프론트엔드가져오기의네트워크요청함수(필요돌아가기promise)
  imports액션?: (data: IObject[]) => Promise<any>;
  // 주요키이름(기본값로id)
  pk?: string;
  // 표工具열(기본값:add,delete,export,也可사용자 정의)
  toolbar?: Array<ToolbarLeft | IToolsButton>;
  // 표工具열右측아이콘(기본값:참조resh,filter,imports,exports,search)
  defaultToolbar?: Array<ToolbarRight | IToolsButton>;
  // table컴포넌트컬럼속성(额外의속성templet,operat,slotName)
  cols: Array<{
...
export interface IModalConfig<T = any> {
  ...
  // 제출의네트워크요청함수(필요돌아가기promise)
  form액션?: (data: T) => Promise<any>;
}
...
export type IFormItems<T = IComponentType> = Array<{
  // 컴포넌트타입(예input,select,radio,custom等)
  type: T;
  // 태그提示
  tips?: string | IObject;
  // 태그텍스트
