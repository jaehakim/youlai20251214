...
export const STORAGE_KEYS = {
  ...
  REMEMBER_ME: `${APP_PREFIX}:auth:remember_me`, // 记住로그인상태

  // 시스템核心관련
  DICT_CACHE: `${APP_PREFIX}:system:dict_cache`, // 사전데이터캐시

  // UI설정관련
  SHOW_TAGS_VIEW: `${APP_PREFIX}:ui:show_tags_view`, // 표시태그页뷰
  SHOW_APP_LOGO: `${APP_PREFIX}:ui:show_app_logo`, // 표시应用Logo
  SHOW_WATERMARK: `${APP_PREFIX}:ui:show_watermark`, // 표시水印
  ENABLE_AI_ASSISTANT: `${APP_PREFIX}:ui:enable_ai_assistant`, // 활성화 AI 도우미
  LAYOUT: `${APP_PREFIX}:ui:layout`, // 레이아웃모드
  SIDEBAR_COLOR_SCHEME: `${APP_PREFIX}:ui:sidebar_color_scheme`, // 측엣지열配色方案
  THEME: `${APP_PREFIX}:ui:theme`, // 테마모드
  THEME_COLOR: `${APP_PREFIX}:ui:theme_color`, // 테마色

  // 应用상태관련
  DEVICE: `${APP_PREFIX}:app:device`, // 设备타입
  SIZE: `${APP_PREFIX}:app:size`, // 屏幕尺寸
  LANGUAGE: `${APP_PREFIX}:app:language`, // 应用언어
  SIDEBAR_STATUS: `${APP_PREFIX}:app:sidebar_status`, // 측엣지열상태
  ACTIVE_TOP_MENU_PATH: `${APP_PREFIX}:app:active_top_menu_path`, // 当前활성화의상단메뉴단일경로
} as const;

export const ROLE_ROOT = "ROOT"; // 슈퍼 관리자역할
